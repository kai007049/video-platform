class DanmuPlayer {
  ...
  updateContainerWidth() {
    this.containerWidth = this.container.clientWidth || 960
  }
  ...
  findAvailableTrack() {
    ...
    for (let i = 0; i < this.trackCount; i++) {
      ...
      if (availableTime <= now) {
        idleTracks.push(i)
      } else if (availableTime < bestTime) {
    ...
    if (idleTracks.length > 0) {
      return idleTracks[Math.floor(Math.random() * idleTracks.length)]
    }
    return bestTrack
  }

  checkTrackOverlap(track, width, speed) {
    const now = performance.now()
    const gapTime = ((width + this.danmuGap) / speed) * 1000
    const nextAvailable = now + gapTime

    for (const [id, danmu] of this.activeDanmus) {
      if (danmu.track === track && danmu.type !== 'top') {
        const danmuEndTime = danmu.startTime + (danmu.duration * 1000)
        if (now < danmuEndTime) {
          const danmuRemainingTime = danmuEndTime - now
          const danmuRemainingDistance = (danmuRemainingTime / 1000) * speed
          const overlapDistance = danmuRemainingDistance + width + this.danmuGap
          if (overlapDistance > this.containerWidth) {
            return true
          }
        }
      }
  ...
  createDanmu(danmu) {
    ...
    let track = this.findAvailableTrack()
    let retryCount = 0
    const maxRetries = this.trackCount

    while (retryCount < maxRetries) {
      if (!this.checkTrackOverlap(track, 100, this.speed)) {
        break
      }
      track = (track + 1) % this.trackCount
      retryCount++
    }
    ...
    const travel = this.containerWidth + width
    ...
    const animate = () => {
      ...
      for (const [id, danmu] of this.activeDanmus) {
        ...
        if (progress >= 1) {
          ...
          danmu.x = this.containerWidth - (progress * (this.containerWidth + danmu.width))
  ...
  setSpeed(speed) {
    ...
    for (const danmu of this.activeDanmus.values()) {
      if (danmu.type !== 'top') {
        danmu.speed = speed
        const remainingTime = (danmu.duration * 1000) - (performance.now() - danmu.startTime)
        const remainingDistance = (remainingTime / 1000) * danmu.speed
        danmu.duration = remainingDistance / speed
      }
    }
  }
  ...
  setFontSize(size) {
    ...
    for (const danmu of this.activeDanmus.values()) {
      ...
      const width = danmu.element.getBoundingClientRect().width || 0
      if (danmu.width !== width) {
        danmu.width = width
        const travel = this.containerWidth + width
        danmu.duration = Math.max(6, travel / danmu.speed)
      }
    }
  }
